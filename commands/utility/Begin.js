...
module.exports.run = async (client, message, args) => {
  ...
  if (
    ...
    !message.member.roles.cache.some(r => r.name === "Hunger Games")
  ) {
    return message.reply(
      ":x: You need to have the manage messages permissions to start Hunger Games."
    );
  }
  ...
  if (isNaN(giveawayNumberWinners) || parseInt(giveawayNumberWinners) <= 0) {
    return message.reply(
      ":x: You have to specify a valid number of contestants!"
    );
  }
