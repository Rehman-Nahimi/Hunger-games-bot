...
testDist.Players.forEach((element) => {
  element.Url = miscScenario.GetScenario(element);
});

console.table(testDist.Players.map((x) => x.Url));


testDist.Players.forEach((element) => {
  element.Url = deathScenario.GetScenario(element);
});
